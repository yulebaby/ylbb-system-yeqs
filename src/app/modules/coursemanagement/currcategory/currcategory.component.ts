...
export class CurrcategoryComponent implements OnInit {
  ...
  //查询课程类型、类别
  selectTypeListQuery() {
    this.http.post('/intelligent/selectLesson', { status: '1', name: this.typeName }, false).then(res => {
      if (res.code == 1000) {
  ...
  isRemoveType() {
    this.http.post('/intelligent/delectLesson', { status: this.Typestatus, id: this.TypeId }, false).then(res => {
      if (res.code == 1000) {
  ...
  editType(type, id) {
    ...
    this.openaddType = true;
    this.openTit = '编辑课程类别';
    this.http.post('/intelligent/selectLessonType', { status: this.addstatus, id: id }, false).then(res => {
      if (res.code == 1000) {
